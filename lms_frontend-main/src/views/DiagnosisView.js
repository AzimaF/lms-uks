export default class DiagnosisView {
  ...
  generatePDF(elementId, filename) {
    ...
    element.classList.remove("animate-fadeIn", "animate-pop-in", "card"); 
  ...
  showFormResult(data, result) {
    ...
    // --- LOGIKA BARU: Generate List Penjelasan HTML ---
    let explanationHTML = '';
  ...
  showImageResult(data) {
    ...
    const redPct = data.shap_stats ? data.shap_stats.red_percentage : "0%";
    const bluePct = data.shap_stats ? data.shap_stats.blue_percentage : "0%";

    // --- LOGIKA BARU: Tentukan Tampilan Statistik Berdasarkan Prediksi ---
    let statsHTML = "";

    if (data.prediction === "Tuberkulosis") {
        // TAMPILAN KHUSUS TBC (HANYA MERAH)
        statsHTML = `
            <div class="text-center animate-fadeIn">
                <p class="text-5xl font-black text-red-500">${redPct}</p>
        ...
        // TAMPILAN KHUSUS NORMAL (HANYA BIRU)
        statsHTML = `
            <div class="text-center animate-fadeIn">
                <p class="text-5xl font-black text-blue-500">${bluePct}</p>
                <div class="flex items-center justify-center gap-2 mt-2">
        ...
        <div class="py-2 flex flex-col items-center">
            ...
            <div class="flex gap-4 flex-wrap justify-center">
                <div class="rounded-2xl overflow-hidden border-2 border-gray-50 shadow-inner bg-black flex justify-center max-w-sm">
                    <img src="data:image/png;base64,${data.shap_img}" alt="SHAP Rontgen Clean" class="w-auto h-64 object-contain" crossorigin="anonymous"/>
                </div>
            </div>
